...
int main(void) {
    ...
    ///Run the BFS
    //Push starting state, mark as visited
    q.push( make_pair(state,0) );
    visited[ getVisitedIndex(r1,c1,r2,c2,r3,c3) ]++;
    while (!q.empty()) {
        //Get front data, mark as visited
        pair<vector< pair<int,int> >,int> Curr = q.front(); q.pop();
        pair<int,int> currOne = Curr.first[0];
        pair<int,int> currTwo = Curr.first[1];
        pair<int,int> currThree = Curr.first[2];
        int currD = Curr.second;
        ///For each child of current
            ///set child's distance as currD+1
                ///if child is solution, return child
            ///if child is not visited yet, add to queue and mark as visited
    }
    traverseMap();
}
